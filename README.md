...
给 DeepSeek Harness 0.1.5-rc.2 根 agent 换一套 Grok Build 风格的 Goal：命令、工具、提示策略、持久状态和输入框旁的 dock。
...
`tsdown.config.ts` 必须继续用 dshx 的 `externalClientBundle`。仓库内的 client bundler 不接受树外的 `my-plugins/*`。
